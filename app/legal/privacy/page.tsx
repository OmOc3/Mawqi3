...
export default function PrivacyPage() {
  return (
    <main className="min-h-dvh bg-slate-50 px-4 py-8 text-right sm:px-6 lg:px-8" dir="rtl">
      <article className="mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-5 shadow-control sm:p-8">
        <BrandLockup className="mb-8" />
        <div className="border-b border-slate-200 pb-6">
          <p className="text-sm font-medium text-teal-700">
            {i18n.legal.privacy} / {i18n.en.legal.privacy}
          </p>
          <h1 className="mt-2 text-2xl font-bold text-slate-900">سياسة خصوصية {BRAND.nameArabic}</h1>
        ...
        <div className="mt-8 space-y-7">
          {sections.map((section) => (
            <section
              className="grid gap-3 border-b border-slate-100 pb-6 last:border-b-0 last:pb-0"
              key={section.titleEn}
            >
              <div>
                <h2 className="text-lg font-semibold text-slate-900">{section.titleAr}</h2>
                <p className="text-sm font-medium text-slate-500" dir="ltr">
                  {section.titleEn}
                </p>
              </div>
              <p className="text-sm leading-7 text-slate-700">{section.bodyAr}</p>
              <p className="text-sm leading-7 text-slate-600" dir="ltr">
                {section.bodyEn}
              </p>
            </section>
          ))}
