...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t.errors.accessDeniedTitle };
}
  ...
  return (
    <main className="flex min-h-dvh items-center px-4 py-6 sm:px-6">
      <section className="mx-auto w-full max-w-md rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 text-center shadow-control sm:p-6">
        ...
        <h1 className="text-2xl font-extrabold text-[var(--foreground)]">{t.errors.accessDeniedTitle}</h1>
        <p className="mt-3 text-base leading-7 text-[var(--muted)]">{t.errors.accessDenied}</p>
        <Link
