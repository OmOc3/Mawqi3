...
export default async function LoginPage() {
  ...
  const locale = await getRequestLocale();
  const t = getI18nMessages(locale);
  const displayName = locale === "ar" ? t.appNameArabic : t.appName;
      ...
      <section className="brand-grid relative hidden min-h-dvh overflow-hidden bg-[var(--sidebar)] text-[var(--sidebar-text)] lg:flex lg:items-center lg:justify-center">
        ...
        <div className="relative z-10 mx-auto max-w-sm px-10 text-center">
          ...
          <div className="mt-8 space-y-3">
            <p className="text-sm font-semibold text-teal-300">{t.appTitle}</p>
            <h1 className="text-3xl font-bold tracking-tight text-[var(--sidebar-text)]">{displayName}</h1>
            <p className="text-sm leading-7 text-[var(--sidebar-muted)]">{t.auth.loginSubtitle}</p>
          </div>
        </div>
      </section>
          ...
          <div className="overflow-hidden rounded-2xl bg-[var(--surface)] shadow-card-lg">
            ...
            <div className="space-y-6 p-5 sm:p-6">
              <div>
                ...
                <p className="mt-2 text-sm leading-7 text-[var(--muted)]">{t.auth.loginSubtitle}</p>
              </div>
              <LoginForm staffPortalLogin />
              <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-subtle)] px-4 py-3 text-center text-sm text-[var(--muted)]">
                {t.auth.staffClientNoticeLead}
                <Link className="font-semibold text-teal-700 hover:underline" href="/client/login">
                  {t.auth.staffClientNoticeLink}
                </Link>
                {t.auth.staffClientNoticeTrail}
              </div>
            </div>
          </div>
