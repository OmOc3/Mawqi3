...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t.errors.accountDisabledTitle };
}
    ...
    <main className="flex min-h-dvh items-center bg-[var(--background)] px-4 py-6 sm:px-6">
      <section className="mx-auto w-full max-w-md">
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 text-center shadow-card sm:p-6">
          ...
          <h1 className="text-2xl font-extrabold text-[var(--foreground)]">{t.errors.accountDisabledTitle}</h1>
          <p className="mt-3 text-base leading-7 text-[var(--muted)]">{t.errors.accountDisabled}</p>
          <p className="mt-2 text-sm text-[var(--muted)]">{t.errors.accountDisabledContact}</p>
          <Link
